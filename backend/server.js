...
app.use(cors());  //to dont have any cors error in client 
app.use(helmet({
  contentSecurityPolicy: false,
}));  // helmet is a security middleware that helps you protect your app 
...
app.use(async (req, res, next) => {
  try {
    ...
    //check for spoofed bots:when bot tries to act like not a bot 
    if (decision.results.some((result) => result.reason.isBot() && result.reason.isSpoofed())) {
      res.status(403).json({ error: "Spoofed bot detected" });
      return;
    }

    next(); //call the next function, any of productRoutes
  } catch (error) {
